...
if int(len(sys.argv) == 5):
   input.nelx = int(sys.argv[1])
   input.nely = int(sys.argv[2])
   visu = int(sys.argv[3])
   slope= float(sys.argv[4])
else:
   visu = 1 
   slope= 1 # between 0.4 and 1, determines whether or not you stretch the mesh (1 = don't stretch the mesh) 
...
print('input.case=',input.case)
print('input.relax=',input.relax)
print('slope=',slope)
...
print('input.nelx=',input.nelx)
print('input.nely=',input.nely)
print('nel=',nel)
#print('hx=',hx)
#print('hy=',hy)
...
for j in range(0,nny):
       for i in range(0,nnx):
           xV[counter]=i*Lx/(2*input.nelx) 
           yV[counter]=j*Ly/(2*input.nely) 
           counter+=1

counter=0
for j in range(0,input.nely):
       for i in range(0,input.nelx):
             # lower left triangle
...
#np.savetxt('output/gridV.ascii',np.array([xV,yV]).T,header='# x,y')
...
for i in range(0,NV):
    ...
    if abs(yV[i]-Ly+xV[i])<1 and xV[i]>=50e3:
       #print('opla')
       bc_fix[i*ndofV  ] = True ; bc_val[i*ndofV  ] = input.slab_vel/np.sqrt(2)
       bc_fix[i*ndofV+1] = True ; bc_val[i*ndofV+1] =-input.slab_vel/np.sqrt(2)
    ...
    #right side
    if abs(xV[i]-Lx)<1 and yV[i]<= Ly-50e3 and input.case=='1b' :
       ui,vi=velocity.compute_corner_flow_velocity(xV[i],yV[i],l1,l2,l3,angle,input.slab_vel,Lx,Ly)
       bc_fix[i*ndofV  ] = True ; bc_val[i*ndofV  ] = ui
       bc_fix[i*ndofV+1] = True ; bc_val[i*ndofV+1] = vi

    #bottom
    if abs(yV[i])<1 and xV[i]>= 600e3 and input.case=='1b' :
       ui,vi=velocity.compute_corner_flow_velocity(xV[i],yV[i],l1,l2,l3,angle,input.slab_vel,Lx,Ly)
       bc_fix[i*ndofV  ] = True ; bc_val[i*ndofV  ] = ui
       #bc_fix[i*ndofV+1] = True ; bc_val[i*ndofV+1] = vi
...
T      = np.full((NT,1),input.Ttop+input.kelvin,dtype=np.float64)           # temperature field
NNNV   = np.zeros(mV,dtype=np.float64)           # shape functions V
...
################################################################################################
################################################################################################
convfile=open('output/conv.ascii',"w")

Tfile=open('output/stats_T.ascii',"w")
ufile=open('output/stats_u.ascii',"w")
vfile=open('output/stats_v.ascii',"w")
pfile=open('output/stats_p.ascii',"w")
Tcorner_file=open('output/stats_Tcorner.ascii',"w")
Tslab_file=open('output/stats_Tslab.ascii',"w")
Twedge_file=open('output/stats_Twedge.ascii',"w")
vrms_file=open('output/vrms.ascii',"w")

for iter in range(0,input.niter):
    ...
    if input.case=='1a':
       for i in range(0,NV):
           u[i],v[i]=velocity.compute_corner_flow_velocity(xV[i],yV[i],l1,l2,l3,angle,input.slab_vel,Lx,Ly)
       #end for
    else:
    ...
    if input.case=='1b' or input.case=='1c' or input.case=='2a' or input.case=='2b':
       for i in range(0,NV):
           if yV[i]>=Ly-50e3: 
              ...
              v[i]=0.
           if yV[i]<Ly-xV[i]: 
              u[i]= input.slab_vel/np.sqrt(2)
              v[i]=-input.slab_vel/np.sqrt(2)
    ...
    print("     -> u (m,M) %.6e %.6e (input.cm/yr)" %(np.min(u)/input.cm*input.year,np.max(u)/input.cm*input.year))
    print("     -> v (m,M) %.6e %.6e (input.cm/yr)" %(np.min(v)/input.cm*input.year,np.max(v)/input.cm*input.year))
    print("     -> p (m,M) %.6e %.6e (MPa)" %(np.min(p)/1e6,np.max(p)/1e6))
    np.savetxt('output/velocity.ascii',np.array([xV,yV,u,v]).T,header='# x,y,u,v')

    print("solve time: %.3f s" % (timing.time() - start))

    #####################################################################
    # input.relaxation step
    #####################################################################

    if iter>0:
       u=input.relax*u+(1-input.relax)*u_old
       v=input.relax*v+(1-input.relax)*v_old
    ...
    # ----- boundary conditions from 1D script ----- # 
    ...
    if input.left_boundary_condition == 1:
        ...
        # resolve the difference in coordinate conventions between the 1D model for the left boundary condition and this code 
        ini_space = Ly - ini_space
        
        # which plate age do we want to use? 
        dummy,nt = np.shape(ini_time)
        for i in range(0,nt):
            if ini_time[0,i] == input.slab_age:
                timestep = i
        ini_temp = temp1D[timestep,:]
        
        # interpolate from regular 1D FD mesh to irregular 1D LHS FE mesh 
        temp_LHS = griddata(ini_space, ini_temp, yT, method='cubic')    
        
    # --- end boundary conditions from 1D script --- #

    for i in range(0,NT):
        # top boundary - vack08
        if abs(yT[i]-Ly)<1: #
           bc_fixT[i]=True ; bc_valT[i]= input.Ttop + input.kelvin
        # left boundary 
        if xT[i]<1:
           bc_fixT[i]=True 
           if input.left_boundary_condition == 0:
               ...
               if yT[i] < np.min(ini_space):
                   ...
                   bc_valT[i]= temp_LHS[i]
        # right boundary 
        if abs(xT[i]-Lx)<1:
           if yT[i]>=Ly-l2:
              bc_fixT[i]=True ; bc_valT[i]=((Ly-yT[i]))/l2*input.Tmax+input.kelvin
           elif u[i]<0:
              bc_fixT[i]=True ; bc_valT[i]=input.Tmax+input.kelvin
    ...
    for iel in range (0,nel):
        ...
        for kq in range(0,nqel):
            ...
            #print(thermal_parameters.heat_conductivity(input.option_k,Tq,pq,mat[iel]))
    ...
    np.savetxt('output/T.ascii',np.array([xT,yT,T]).T,header='# x,y')
    ...
    if iter>0:
       T=input.relax*T+(1-input.relax)*T_old
    ...
    print("conv: u,v,T: %.6f %.6f %.6f | input.tol= %.6f" %(xi_u,xi_v,xi_T,input.tol))

    convfile.write("%3d %10e %10e %10e %10e \n" %(iter,xi_u,xi_v,xi_T,input.tol)) 
    convfile.flush()

    if xi_u<input.tol and xi_v<input.tol and xi_T<input.tol:
       break
    else:
       u_old[:]=u[:]
       v_old[:]=v[:]
    ...
    if visu:
       filename = 'output/Vnodes_{:04d}.vtu'.format(iter)
       ...
       filename = 'output/qpoints_{:04d}.vtu'.format(iter)
       vtufile=open(filename,"w")
       vtufile.write("<VTKFile type='UnstructuredGrid' version='0.1' byte_order='BigEndian'> \n")
       vtufile.write("<UnstructuredGrid> \n")
       vtufile.write("<Piece NumberOfPoints=' %5d ' NumberOfCells=' %5d '> \n" %(nq,nq))
    ...
    print('     -> vrms=',vrms,input.nelx,np.sqrt(2*np.min(area)))
...
diagfile=open('output/tempdiag.ascii',"w")
for i in range(0,NT):
    if abs(yT[i] -Ly+ xT[i] ) <1: 
       diagfile.write("%10e %10e %10e \n " %(xT[i],yT[i],T[i]-input.kelvin))
diagfile.close()
...
print ('     -> Tavrg=',Tavrg,input.nelx,np.sqrt(2*np.min(area)))
...
if do_post_processing:
   ...
   for j in range(0,nnny):
       for i in range(0,nnnx):
           ...
           grid_x[counter]=min(grid_x[counter],(1-input.eps)*Lx)
           grid_x[counter]=max(grid_x[counter],input.eps*Lx)
           grid_y[counter]=max(grid_y[counter],input.eps*Ly)
           grid_y[counter]=min(grid_y[counter],(1-input.eps)*Ly)
           counter += 1
       #end for
   #end for
   ...
   for i in range(0,M):
       if slope==1:
          ielx=int(grid_x[i]/Lx*input.nelx)
          iely=int(grid_y[i]/Ly*input.nely)
          iel1=2*(iely*input.nelx+ielx)
          iel2=2*(iely*input.nelx+ielx)+1
          #local coords between 0,1 in square cell
          r=(grid_x[i]-xT[iconT[0,iel1]])/hx
          s=(grid_y[i]-yT[iconT[0,iel1]])/hy
       else:
          for ielx in range(0,input.nelx):
              if grid_x[i]>=xV[iconV[0,2*ielx]] and grid_x[i]<=xV[iconV[1,2*ielx]]:
                 break
          for iely in range(0,input.nely):
              if grid_y[i]>=yV[iconV[0,2*iely*input.nelx]] and grid_y[i]<=yV[iconV[2,2*iely*input.nelx]]:
                 break
   ...
   grid_T1-=input.kelvin
   grid_T2-=input.kelvin

   #temperature $T(11,11)$ which is the 111+11=122th point
   inode=111*10+11-1
   grid_corner[inode]=1
   print('     -> Tcorner=',grid_T1[inode],grid_T2[inode],grid_x[inode],grid_y[inode],input.nelx,np.sqrt(2*np.min(area)))
   ...
   print('     -> Tslab=',Tslab,input.nelx,np.sqrt(2*np.min(area)))
   ...
   print('     -> Twedge=',Twedge,input.nelx,np.sqrt(2*np.min(area)))

   #Twedge_file.write("%3d %10e \n " %(iter,Twedge))
   #Twedge_file.flush()

   np.savetxt('output/meas_grid.ascii',np.array([grid_x,grid_y,grid_T1,grid_T2,grid_corner,grid_slab,grid_wedge]).T)

   if visu:
      filename = 'output/meas_grid.vtu'
      vtufile=open(filename,"w")
      vtufile.write("<VTKFile type='UnstructuredGrid' version='0.1' byte_order='BigEndian'> \n")
      vtufile.write("<UnstructuredGrid> \n")
      vtufile.write("<Piece NumberOfPoints=' %5d ' NumberOfCells=' %5d '> \n" %(M,M))
...
if visu:
    vtufile=open('output/solution.vtu',"w")
    vtufile.write("<VTKFile type='UnstructuredGrid' version='0.1' byte_order='BigEndian'> \n")
    vtufile.write("<UnstructuredGrid> \n")
    vtufile.write("<Piece NumberOfPoints=' %5d ' NumberOfCells=' %5d '> \n" %(nnx*nny,nel))
    #####
    ...
    for i in range(0,NT):
        vtufile.write("%10e %10e %10e \n" %(u[i]/input.cm*input.year,v[i]/input.cm*input.year,0.))
    vtufile.write("</DataArray>\n")
    #--
    vtufile.write("<DataArray type='Float32' NumberOfComponents='3' Name='velocity (diff)' Format='ascii'> \n")
    for i in range(0,NT):
        vtufile.write("%10e %10e %10e \n" %((u[i]-u_old[i])/input.cm*input.year,(v[i]-v_old[i])/input.cm*input.year,0.))
    vtufile.write("</DataArray>\n")
    #--
    ...
    for i in range(0,NV):
        if bc_fix[i*2]:
           val=bc_val[i*2]/input.cm*input.year
        else:
           val=0
        vtufile.write("%10e \n" %val)
    vtufile.write("</DataArray>\n")
    ...
    for i in range(0,NV):
        if bc_fix[i*2+1]:
           val=bc_val[i*2+1]/input.cm*input.year
        else:
           val=0
        vtufile.write("%10e \n" %val)
    vtufile.write("</DataArray>\n")
    ...
    vtufile=open('output/gridP.vtu',"w")
    vtufile.write("<VTKFile type='UnstructuredGrid' version='0.1' byte_order='BigEndian'> \n")
    vtufile.write("<UnstructuredGrid> \n")
    vtufile.write("<Piece NumberOfPoints=' %5d ' NumberOfCells=' %5d '> \n" %(NfemP,nel))
    #####
